...
wtsn_error wtsn_db_qos_save(wtsn_db *db, const wtsn_qos_config *cfg) {
    ...
    const char *sql =
        ...
        " latency_ms=excluded.latency_ms, preemption=excluded.preemption;";
    ...
    sqlite3_bind_int(st, 6, cfg->preemption);
...
wtsn_error wtsn_db_qos_load(wtsn_db *db, const char *device_id, wtsn_qos_config *out) {
    ...
    sqlite3_stmt *st = NULL;
    const char *sql =
        "SELECT device_id,priority,traffic_class,bandwidth_kbps,latency_ms,preemption"
    ...
    out->preemption = sqlite3_column_int(st, 5);
    sqlite3_finalize(st);
    return WTSN_OK;
}
